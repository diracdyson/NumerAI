...
# interesting 
    def VIF(self,c =''):
        ...
        vif.plot(kind='barh',xlabel='Factor',ylabel ='Features',title='VIF Multicolinearity',colorbar ='reds')
...
# reduce feature exposure aka overfitting
   ...
   # era based neut
   @staticmethod
    def NormNeutralizedPredGroup(df,neut_col,y_pred,proportion)->np.array():
    ...
    @staticmethod
    def RiskiestFeat(df,y_true,y_pred,top_count) -> list():
        all_features_corrs= df.groupby('era').apply(lambda x: x.corrwith(y_true))
        feature_corr_vol = all_feature_corrs.std() 
        feature_exposure_list=[]
        for feature in df.columns.drop('era',axis=1).columns:
            feature_exposure_list.append(self.CorrelationScore(df[feature],y_pred))
...
# neut riskiest feat, cosnider the vol of the corr of each feat with target                
            ...
            predneut_l = self.NormNeutralizedPredGroup(self.X_live.drop('id',axis=1),self.riskyF,pred_l,proportion)
            predneut_c = self.NormNeutralizedPredGroup(self.X_live.drop('id',axis=1),self.riskF,pred_c,proportion)
