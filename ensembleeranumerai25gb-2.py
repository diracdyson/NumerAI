...
class EnsembleModel():
  ...
  #      y_val =X_val['target']
   ...
   #     self.X_v= self.X_v.drop(targetfeat,axis=1)
       ...
       # print('error')
        if test_app:
            self.X_live= pd.read_parquet("/content/drive/My Drive/CURRDATA/liveera.csv")
...
#
 ...
 #   pred2t=interpscale(pred_lt,0 ,1 )
    ...
    def PredictSubmitEnsembleOverEra(self,file_name,model_id,n_splits=3,proportion_m = 0.1,FeatImpSelection=True,up=False,ms=2):
     ...
     # neut riskiest feat, cosnider the vol of the corr of each feat with target                
                ...
                pred_ct+= model_c.predict(self.X_live)/ms 
